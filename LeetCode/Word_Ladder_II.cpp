...
class CWord_Ladder_II {
	...
	{
		int level;
		list<stdext::hash_set<string>::iterator> list_prev;
		int useNum;
		__level_node__():level(0),useNum(0){}
	}LEVEL_NODE;
public:
	vector<vector<string>> findLadders(string start, string end, stdext::hash_set<string> &dict) {
		...
		stdext::hash_set<string>::iterator hash_it;
		while (0 != parent_num)
		{
				...
				if (0 != map_ladder[end].level)
				{
					...
					while(1 < level)
					{
						...
						for (int i = 0; i < parent_num; ++i)
						{
							...
							list<stdext::hash_set<string>::iterator>::iterator list_it = map_ladder[temp_str].list_prev.begin();
